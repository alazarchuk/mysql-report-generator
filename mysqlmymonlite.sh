...
HTTPDCHECK=`ps ax | $GREP -v $GREP | $GREP apache2`

if [[ $HTTPDCHECK ]]; then

APACHECTL="$(which apachectl)"
#if [ -s /usr/bin/lynx ]; then
#HTTPDFULLSTATUS=`/usr/bin/lynx -dump http://127.0.0.1/server-status | $GREP -E '(Server Version|Server Built|Current Time|Restart Time|Parent Server Generation|Server Uptime|Total accesses|CPU Usage|requests/sec -|requests currently being processed)'`
#else
#apt-get -y -q install lynx
#fi

HTTPDFULLSTATUS=`$APACHECTL fullstatus | $GREP -E '(Server Version|Server Built|Current Time|Restart Time|Parent Server Generation|Server Uptime|Total accesses|CPU Usage|requests/sec -|requests currently being processed)'`
...
echo ""
echo "installing which via apt-get"
echo "apt-get -y install which"
...
MYSQLCOMPILE=`$GREP -E '(^COMP_CALL_INFO|^CONFIGURE_LINE)' /usr/bin/mysqlbug`
...
MYSQLSTART=`$MYSQLCLI $MYSQLADMINOPT -e "SELECT FROM_UNIXTIME(UNIX_TIMESTAMP() - variable_value) AS server_start FROM INFORMATION_SCHEMA.GLOBAL_STATUS WHERE variable_name='Uptime';" | $GREP -Ev '+--|server_start'`
...
php -i | $GREP -E '(display_errors|error_log|extension_dir|file_uploads|magic_quotes_gpc|log_errors|magic_quotes_sybase|max_execution_time|memory_limit|open_basedir|post_max_size|realpath_cache_size|realpath_cache_ttl|safe_mode|sendmail_path|upload_max_filesize|upload_tmp_dir)' | sed -e 's/=>/|/g' && echo ""

php -i | $GREP -E '(^cURL|^GD|^FreeType|^GIF Read|^GIF Create|^JPEG Support|^libJPEG Version|^PNG Support|^libPNG Version|^WBMP Support|^XBN Support|^libXML|^PCRE|^Soap|^ZLib)' | sed -e 's/=>/|/g' && echo ""
...
php -i | $GREP -E '(^suhosin.get.max_vars|^suhosin.post.max_vars|^suhosin.post.max_value_length|^suhosin.request.max_vars|^suhosin.request.max_value_length|^suhosin.executor.disable_eval|^suhosin.cookie.encrypt|^suhosin.memory_limit|^suhosin.get.max_value_length|^suhosin.upload.disallow_binary|^suhosin.upload.disallow_elf|^suhosin.upload.max_uploads|^suhosin.upload.remove_binary|^suhosin.upload.verification_script)' | sed -e 's/=>/|/g' && echo ""
php -i | $GREP -E '(Apache Version|Max Requests|Timeouts|Loaded Modules)' | sed -e 's/=>/|/g' && echo ""
...
$LSPHPBIN -i | $GREP -E '(display_errors|error_log|extension_dir|file_uploads|magic_quotes_gpc|log_errors|magic_quotes_sybase|max_execution_time|memory_limit|open_basedir|post_max_size|realpath_cache_size|realpath_cache_ttl|safe_mode|sendmail_path|upload_max_filesize|upload_tmp_dir)' | sed -e 's/=>/|/g' && echo ""

$LSPHPBIN -i | $GREP -E '(^cURL|^GD|^FreeType|^GIF Read|^GIF Create|^JPEG Support|^libJPEG Version|^PNG Support|^libPNG Version|^WBMP Support|^XBN Support|^libXML|^PCRE|^Soap|^ZLib)' | sed -e 's/=>/|/g' && echo ""
$LSPHPBIN -i | awk '/APC|apc|xcache|memcache/' | sed -e 's/=>/|/g' && echo ""

$LSPHPBIN -i | $GREP -E '(^suhosin.get.max_vars|^suhosin.post.max_vars|^suhosin.post.max_value_length|^suhosin.request.max_vars|^suhosin.request.max_value_length|^suhosin.executor.disable_eval|^suhosin.cookie.encrypt|^suhosin.memory_limit|^suhosin.get.max_value_length|^suhosin.upload.disallow_binary|^suhosin.upload.disallow_elf|^suhosin.upload.max_uploads|^suhosin.upload.remove_binary|^suhosin.upload.verification_script)' | sed -e 's/=>/|/g' && echo ""
$LSPHPBIN -i | $GREP -E '(Apache Version|Max Requests|Timeouts|Loaded Modules)' | sed -e 's/=>/|/g' && echo ""
...
echo ""
$GREP -E '(^<IfModule mpm_prefork_module>|StartServers|MinSpareServers|MaxSpareServers|ServerLimit|MaxClients|MaxRequestsPerChild|^</IfModule>|^<IfModule mpm_worker_module>|MaxClients|MinSpareThreads|MaxSpareThreads|ThreadsPerChild)' $HTTPDCONF | uniq | $GREP -Ev '#'
echo ""
...
echo ""
$GREP -E '(^<IfModule prefork.c>|^StartServers|^MinSpareServers|^MaxSpareServers|^ServerLimit|^MaxClients|^MaxRequestsPerChild|^</IfModule>|^<IfModule worker.c>|^MaxClients|^MinSpareThreads|^^MaxSpareThreads|^ThreadsPerChild)' $HTTPDMPMCONF | uniq
echo ""
...
DBIDXSIZE=`$MYSQLCLI $MYSQLADMINOPT -e "SELECT CONCAT(ROUND(SUM(index_length)/(1024*1024), 2), ' MB') AS 'Total Index Size' FROM information_schema.TABLES WHERE table_schema LIKE '$dbname';" | $GREP -Ev '(+-|Total Index Size)'`
...
FROM information_schema.TABLES WHERE table_schema LIKE '$dbname';" | $GREP -Ev '(+-|Total Data Size)'`
if [[ "$MASKDB" = [yY] ]]; then
...
TABLENAME=`$MYSQLCLI $MYSQLADMINOPT -e "SELECT table_name FROM information_schema.tables WHERE table_schema LIKE '$mysqldbname';" | $GREP -Ev '(table_name|+---)'`
...
FROM information_schema.TABLES WHERE table_schema LIKE '$mysqldbname';" | $GREP -Ev '(+-|Total Data Size)'`

VBDBIDXSIZE=`$MYSQLCLI $MYSQLADMINOPT -e "SELECT CONCAT(ROUND(SUM(index_length)/(1024*1024), 2), ' MB') AS 'Total Index Size' FROM information_schema.TABLES WHERE table_schema LIKE '$mysqldbname';" | $GREP -Ev '(+-|Total Index Size)'`
